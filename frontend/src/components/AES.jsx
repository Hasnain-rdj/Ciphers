...
function AES() {
  ...
  const handleEncrypt = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${API_URL}/aes/encrypt`, {
        plaintext,
        key
      });
  ...
  const handleDecrypt = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${API_URL}/aes/decrypt`, {
        ciphertext,
        key
      });
      setResult(response.data);
  ...
  return (
    <div className="cipher-container">
      <div className="cipher-header">
        <h2>AES-128 Encryption</h2>
        <p>Advanced Encryption Standard with 128-bit key</p>
      </div>

      <div className="info-banner">
        <Info size={18} />
        <span>AES uses 128-bit key (MD5 hash of your input key). Output is in hexadecimal format.</span>
      </div>
                  ...
                  <div key={i} className="step-item-detailed">
                    ...
                    <div className="step-content">
                      {step.data && typeof step.data === 'string' && (
                        <div className="hex-value">{step.data}</div>
                      )}
                      {step.data && Array.isArray(step.data) && (
                        <div className="subbytes-grid">
                          {step.data.map((byte, idx) => (
                            <div key={idx} className="byte-item">
                              {byte.original}
                            </div>
                          ))}
                        </div>
