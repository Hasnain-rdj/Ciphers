...
const PlayfairCipher = require('../algorithms/playfair');
...
router.post('/encrypt', (req, res) => {
  try {
    ...
    // Validate plaintext
    const plaintextValidation = ValidationHelper.validateClassicalPlaintext(plaintext, 'Playfair cipher');
    if (!plaintextValidation.valid) {
      return res.status(400).json({ error: plaintextValidation.message });
    }

    // Validate key
    const keyValidation = ValidationHelper.validateClassicalKey(key, 'Playfair cipher', 1);
